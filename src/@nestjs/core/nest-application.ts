...
export class NestApplication {
          ...
          if (methodMetadata) {
            ...
            this.app[methodMetadata.toLowerCase()](
              ...
              async (
                ...
                const responseMeta = this.getResponseMetadata(
                  controller,
                  methodName
                );
  ...
  private isModule(token) {
    return (
      ...
      Reflect.getMetadata("isModule", token)
    );
  }
  ...
  private addProvider(provider, module, isGlobal = false) {
    const providers = isGlobal
      ? this.globalProviderTokens
      : this.moduleProviderTokens.get(module) ?? new Set();
  ...
  private getResponseMetadata(instance: any, methodName: string): any {
    ...
    return paramMetadata.find((param) =>
      ["Response", "Res"].includes(param.key)
    );
  }
